...
#: Both array-level schemes, which must agree about the rank of any input.
SCHEMES = [
    pytest.param(find_independent_tensors_gram_schmidt, id="gram_schmidt"),
    pytest.param(find_independent_tensors_scipy_qr, id="scipy_qr"),
]

#: Only Gram-Schmidt walks the tensors in the order given, so only it can be asked
#: *which* of several equally valid subsets comes back. Pivoted QR reorders the
#: columns by residual norm, so the subset it picks is its own business.
ORDER_PREFERRING = [
    pytest.param(find_independent_tensors_gram_schmidt, id="gram_schmidt"),
]
...
@pytest.mark.parametrize("scheme", ORDER_PREFERRING)
